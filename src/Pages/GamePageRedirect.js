...
const GamePageRedirect = () => {
  ...
  const [userdetails, setuserdetails] = useState({ firstuser: "", secondUser: "" })
  ...
  const handleStartGame = () => {
    ...
    if (secondUsername && cellCount && FirstUsername) {
      let UsersDetails = userData.map((item) => {
        
        if (item.user.userName == FirstUsername) {
          setuserdetails({ ...userdetails, firstuser: item.user })
        } else if (item.user.userName == secondUsername) {
          setuserdetails({ ...userdetails, secondUser: item.user })
        }
      })
      console.log("details",userdetails)
      localStorage.setItem('gamedetails', JSON.stringify({ userdetails: userdetails, cellCount: cellCount }))
